...
class rater:
    ...
    def train(self, agent):
        ...
        for agent in self.agents:
            ...
            for x in range(2, NUM_EPOCHS):
                print(f'epoch {x}:')
                self.training_history[agent].append(self.agents[agent].fit(
                    Xtrain, ytrain, batch_size=BATCH_SIZE, validation_data=(Xval, yval)))
                self.agents[agent].save(f'models/{agent} {x}.keras')
                with open(f'training history/{agent} {x}.json', 'w') as f:
                    json.dump(self.training_history[agent][x].history, f)

            print('\ntest:')
            self.agents[agent].evaluate(Xtest, ytest, batch_size=BATCH_SIZE, verbose=VERBOSE)

    def load(self, models):
        self.agents['LSTM'] = load_model(f'models/{models[0]}.keras')
        # for name in models:
